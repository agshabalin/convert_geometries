...
class PolySim:
    ...
    def load_inp(self, directory = None, inp_file = None):
        ...
        self.add_to_log(f'Reading file: {self.directory+self.inp_file}', True)
        if not os.path.isfile(self.directory+self.inp_file): raise FileNotFoundError
        
        GM = Geometry(directory = directory)
        ...
        with open(self.directory+self.inp_file, "r") as f:
            for line in f:
                ...
                if ('beamflux' in words[0]):
                    ...
                    GM.set_attr('unitcell', [float(v) for v in words[1:7]])
                elif ('sgno' in words[0]):
                    GM.set_attr('spacegroup', int(words[1]))
    ...
    def save_inp(self, directory = None, inp_file = None, overwrite = False):
        ...
        self.add_to_log(f'Writing file: {self.directory+self.inp_file}', True)
        ...
        f.write('\n### Structural\n')
        try:
            s =[f'{v:.6f}' for v in self.geometry.unitcell]
        except:
            s = ['None']*6
        f.write('unit_cell '+' '.join(s) + '\n' )
        f.write('sgno  {}               # space group number\n'.format(self.geometry.spacegroup))
