...
class Geometry:
    ...
    def load_yml(self, directory = None, yml_file = None, det_num = 1):
        ...
        translation = pars['detectors'][det_name]['transform']['translation']*1000 # in um
        ...
        det_pixsize_sf = np.asarray([pixels['size'][0], pixels['size'][1]])*1000 # in um
        ...
        self.set_attr('distance' , fable_distance)
        self.set_attr('y_size'   , fable_pixsize_sf[1])
        self.set_attr('z_size'   , fable_pixsize_sf[0])
    ...
    def average_geometries(list_of_geometries):
        ...
        GM = L[0]
        n = len(L)
        a = nanmean([g.wavelength for g in L if g.wavelength])
        GM.wavelength = None if a==np.nan else a
        
        a = nanmean([g.unitcell[0] for g in L if g.unitcell[0]])
        GM.unitcell[0] = None if a==np.nan else a
        
        a = nanmean([g.unitcell[1] for g in L if g.unitcell[1]])
        GM.unitcell[1] = None if a==np.nan else a
        
        a = nanmean([g.unitcell[2] for g in L if g.unitcell[2]])
        GM.unitcell[2] = None if a==np.nan else a
        
        a = nanmean([g.distance for g in L if g.distance])
        GM.distance = None if a==np.nan else a

        a = nanmean([g.tilt[0] for g in L if g.tilt[0]])
        GM.tilt[0] = None if a==np.nan else a

        a = nanmean([g.tilt[1] for g in L if g.tilt[1]])
        GM.tilt[1] = None if a==np.nan else a

        a = nanmean([g.tilt[2] for g in L if g.tilt[2]])
        GM.tilt[2] = None if a==np.nan else a

        a = nanmean([g.t[0] for g in L if g.t[0]])
        GM.t[0] = None if a==np.nan else a

        a = nanmean([g.t[1] for g in L if g.t[1]])
        GM.t[1] = None if a==np.nan else a

        a = nanmean([g.t[2] for g in L if g.t[2]])
        GM.t[2] = None if a==np.nan else a
        
        a = nanmean([g.y_center for g in L if g.y_center])
        GM.y_center = None if a==np.nan else a

        a = nanmean([g.z_center for g in L if g.z_center])
        GM.z_center = None if a==np.nan else a
